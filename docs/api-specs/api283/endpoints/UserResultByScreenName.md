...
**Route:** `GET /UserResultByScreenName`
**API:** `twitter283.p.rapidapi.com`
**Description:** User By Screen Name
...
```typescript
// Root: UserByScreenNameResponse
interface UserByScreenNameResponse {
  data: UserByScreenNameResponseData;
}

interface UserByScreenNameResponseData {
  user_results: UserByScreenNameResponseDataUserResults;
}

interface UserByScreenNameResponseDataUserResults {
  rest_id: string;
  result: User;
}
